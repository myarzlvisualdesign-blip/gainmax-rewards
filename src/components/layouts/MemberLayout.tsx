...
import { LogOut, Users, History, Wallet, Home, Crown, Package } from "lucide-react";
      ...
      <div className="bg-gradient-to-br from-primary to-primary/80 px-4 pb-8 pt-6 text-primary-foreground">
        <div className="mx-auto max-w-lg">
          ...
          <div className="rounded-2xl bg-primary-foreground/10 p-5 backdrop-blur-sm">
            ...
            <p className="text-3xl font-extrabold">Rp {totalSaldo.toLocaleString("id-ID")}</p>
            <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
              <div><p className="opacity-70">Bisa Ditarik</p><p className="font-bold">Rp {(profile?.saldo_bisa_ditarik || 0).toLocaleString("id-ID")}</p></div>
              <div><p className="opacity-70">Terkunci</p><p className="font-bold">Rp {(profile?.saldo_terkunci || 0).toLocaleString("id-ID")}</p></div>
              <div><p className="opacity-70">Referral</p><p className="font-bold">Rp {(profile?.saldo_referral || 0).toLocaleString("id-ID")}</p></div>
              <div><p className="opacity-70">Affiliate</p><p className="font-bold">Rp {(profile?.saldo_affiliate || 0).toLocaleString("id-ID")}</p></div>
            </div>
          </div>
        </div>
      </div>

      <div className="mx-auto max-w-lg px-4 py-6"><Outlet /></div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 border-t border-border bg-card">
        <div className="mx-auto flex max-w-lg items-center justify-around py-2">
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={`flex flex-col items-center gap-1 px-3 py-1 text-xs font-medium transition-colors ${active ? "text-primary" : "text-muted-foreground"}`}>
